...
import {
  StructogramView,
  type StructogramExportControls
} from "../structogram/StructogramView";
...
export const DiagramPanel = ({
  ...
  const diagramExportControlsRef = useRef<ExportControls | null>(null);
  ...
  return (
    <ContextMenu>
            ...
            {viewMode === "uml" && graph && diagram ? (
              <UmlDiagram
                ...
                onRegisterExport={(controls) => {
                  diagramExportControlsRef.current = controls;
                  onRegisterExport?.(controls);
                }}
              />
            ) : null}
      ...
      {viewMode === "structogram" ? (
        <ContextMenuContent>
          <ContextMenuItem
            disabled={!resolvedMethodContext}
            onSelect={() => structogramExportControlsRef.current?.copyStructogramPng()}
          >
          ...
          <ContextMenuItem
            disabled={!resolvedMethodContext}
            onSelect={() => structogramExportControlsRef.current?.exportStructogramPng()}
          >
            <span className="inline-flex items-center gap-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        <ContextMenuContent>
          ...
          <ContextMenuSub>
            ...
            <ContextMenuSubContent>
              <ContextMenuItem
                ...
                onSelect={() => diagramExportControlsRef.current?.copyDiagramPng("uncompiled")}
              >
                Uncompiled
              </ContextMenuItem>
              <ContextMenuItem
                disabled={!canExportDiagram}
                onSelect={() => diagramExportControlsRef.current?.copyDiagramPng("compiled")}
              >
                Compiled
              </ContextMenuItem>
            </ContextMenuSubContent>
          ...
          <ContextMenuSub>
            ...
            <ContextMenuSubContent>
              <ContextMenuItem
                disabled={!canExportDiagram}
                onSelect={() => diagramExportControlsRef.current?.exportDiagramPng("uncompiled")}
              >
                Uncompiled
              </ContextMenuItem>
              <ContextMenuItem
                disabled={!canExportDiagram}
                onSelect={() => diagramExportControlsRef.current?.exportDiagramPng("compiled")}
              >
                Compiled
              </ContextMenuItem>
            </ContextMenuSubContent>
